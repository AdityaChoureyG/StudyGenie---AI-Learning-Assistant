...
const ChatInterface = () => {
    ...
    const scrollToBottom = () => {
        messageEndRef.current?.scrollIntoView({ behaviou: "smooth"} );
    }
    ...
    const hanldeSendMessage = async (e) => {
        ...
        try{
            ...
            const errorMessage = {
                role: 'assistant',
                content: 'Sorry, I encountered an error. Please try agian',
                timestamp: new Date() 
            }
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden ">
        ...
        <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50 ">
            {history.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-sm text-slate-500">Ask me anything about the doucment!</p>
                </div>
            ) : (
        ...
        <div className="p-5 border-t border-slate-200/60 bg-white/80">
            <form action="" className="flex items-center gap-3" onSubmit={hanldeSendMessage}>
                <input 
                    type="text" name="" id="" 
                    value={message}
                ...
                <button 
                    ...
                    className='w-12 h-12 shrink-0 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 flex items-center justify-center'
                >
